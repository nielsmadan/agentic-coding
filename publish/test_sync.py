...
class OverrideTest(unittest.TestCase):
    def setUp(self):
        ...
        self.out.mkdir()
        for name in ("demo", "other", "hidden"):
            self._write_skill(
                self.sources, name, f"Default {name}.", "Default workflow."
            )
    ...
    def _render(self, skill, harness):
        banner = f"<!-- Generated by loadout from skills/{skill.name}/. -->\n"
        return skill.document.read_text(encoding="utf-8").replace(
            "\n---\n", f"\n---\n\n{banner}", 1
        )
    ...
    def _build(self):
        return sync.build(
            ...
            skills_root=self.sources,
            loader=lambda: (self._discover, self._render),
        )

    def _assert_rejected(self, message):
        self.assertIn(
            message,
            sync.check_manifest(manifest_path=self.manifest, skills_root=self.sources),
        )
        self.assertIn(
            message,
            sync.source_errors(manifest_path=self.manifest, skills_root=self.sources),
        )
    ...
    def test_override_replaces_whole_directory_and_removal_restores_default(self):
        default = self.sources / "demo"
        (default / "notes.md").write_text("Default notes", encoding="utf-8")
        (default / "personal.md").write_text("Personal workflow", encoding="utf-8")
        self.assertEqual(self._build(), ([], []))
        other = (self.out / "skills/other/SKILL.md").read_bytes()
        source_files = {path.name: path.read_bytes() for path in default.iterdir()}

        override = self._write_skill(
            self.overrides, "demo", "Public demo.", "Public workflow."
        )
        (self.overrides / "README.md").write_text("Public variants", encoding="utf-8")
        (override / "notes.md").write_text("Public notes", encoding="utf-8")
        (override / "THIRD_PARTY.md").write_text("Upstream notice", encoding="utf-8")
        script = override / "scripts/run.sh"
        script.parent.mkdir()
        script.write_text("#!/bin/sh\nprintf 'public\\n'\n", encoding="utf-8")
        script.chmod(0o755)

        self.assertEqual(self._build(), ([], []))
        published = self.out / "skills/demo"
        self.assertEqual(
            sorted(
            ...
            ["SKILL.md", "THIRD_PARTY.md", "notes.md", "scripts/run.sh"],
        )
        for relative in ("notes.md", "THIRD_PARTY.md", "scripts/run.sh"):
            self.assertEqual(
                (published / relative).read_bytes(),
                (override / relative).read_bytes(),
            )
        self.assertEqual((published / "scripts/run.sh").stat().st_mode & 0o777, 0o755)
        document = (published / "SKILL.md").read_text(encoding="utf-8")
        self.assertEqual(
            sync.parse_frontmatter(document)["description"], "Public demo."
        )
        self.assertTrue(document.endswith("Public workflow.\n"))
        self.assertIn(
            "| `demo` | Public demo. |",
            (self.out / "README.md").read_text(encoding="utf-8"),
        )
        self.assertEqual((self.out / "skills/other/SKILL.md").read_bytes(), other)
        self.assertEqual(
            {path.name: path.read_bytes() for path in default.iterdir()}, source_files
        )

        shutil.rmtree(override)
        self.assertEqual(self._build(), ([], []))
        self.assertEqual(
            sorted(path.name for path in published.iterdir()), sorted(source_files)
        )
        self.assertEqual(
            (published / "notes.md").read_text(encoding="utf-8"), "Default notes"
        )
        self.assertIn(
            "| `demo` | Default demo. |",
            (self.out / "README.md").read_text(encoding="utf-8"),
    ...
    def test_source_checks_use_public_version_and_keep_checking_other_skills(self):
        self._write_skill(self.sources, "demo", "Personal demo.", "Use ~/ac.")
        (self.sources / "demo/notes.md").write_text("~/wrksp", encoding="utf-8")
        self._write_skill(
            self.overrides, "demo", "Public demo.", "Use the project root."
        )
        self.assertEqual(
            sync.source_errors(manifest_path=self.manifest, skills_root=self.sources),
            [],
        ...
        self.assertEqual(self._build(), ([], []))
        (self.sources / "other/notes.md").write_text("~/ac", encoding="utf-8")
        self.assertEqual(
            sync.source_errors(manifest_path=self.manifest, skills_root=self.sources),
            ["other/notes.md:1: personal string '~/ac'"],
        )

    def test_override_publishes_when_local_source_is_absent(self):
        self._write_skill(
            self.overrides, "demo", "Public demo.", "Public workflow."
        )
        shutil.rmtree(self.sources / "demo")
        self.assertEqual(
            sync.check_manifest(manifest_path=self.manifest, skills_root=self.sources),
            [],
        )
        ...
        self.assertEqual(self._build(), ([], []))
        document = (self.out / "skills/demo/SKILL.md").read_text(encoding="utf-8")
        self.assertTrue(document.endswith("Public workflow.\n"))

    def test_source_checks_report_personal_strings_in_override_files(self):
        override = self._write_skill(
            self.overrides, "demo", "Public demo.", "Public workflow."
        )
        for relative in ("SKILL.md", "references/notes.md", "scripts/run.sh"):
            with self.subTest(relative=relative):
                path = override / relative
                path.parent.mkdir(parents=True, exist_ok=True)
                original = path.read_bytes() if path.exists() else None
                path.write_text("Use ~/ac\n", encoding="utf-8")
                self.assertEqual(
                    sync.source_errors(
                        manifest_path=self.manifest, skills_root=self.sources
                    ),
                    [f"overrides/demo/{relative}:1: personal string '~/ac'"],
                )
                if original is None:
                    path.unlink()
                else:
    ...
    def test_unpublished_overrides_are_rejected(self):
        for name in ("unknown", "hidden", "local-only"):
            with self.subTest(name=name):
                directory = self._write_skill(
                    self.overrides, name, "An override.", "Body."
                )
                self._assert_rejected(
                    f"overrides/{name}: skill is not listed in a publish group"
                )
                shutil.rmtree(directory)

    def test_incomplete_override_is_rejected_without_falling_back(self):
        (self.overrides / "demo").mkdir(parents=True)
        self._assert_rejected("overrides/demo: missing SKILL.md")

    def test_file_in_place_of_override_is_rejected(self):
        self.overrides.mkdir()
        (self.overrides / "demo").write_text("Incomplete override", encoding="utf-8")
        self._assert_rejected("overrides/demo: expected a skill directory")

    def test_file_in_place_of_overrides_root_is_rejected(self):
        self.overrides.write_text("Wrong path", encoding="utf-8")
        self._assert_rejected("overrides: expected a directory")

    def test_symlinked_override_is_rejected(self):
        self.overrides.mkdir()
        (self.overrides / "demo").symlink_to(self.sources / "demo")
        self._assert_rejected("overrides/demo: symlinked directory refused")

    def test_symlinked_override_document_is_rejected(self):
        directory = self.overrides / "demo"
        directory.mkdir(parents=True)
        (directory / "SKILL.md").symlink_to(self.sources / "demo/SKILL.md")
        self._assert_rejected("overrides/demo: symlinked SKILL.md refused")

    def test_symlinked_overrides_root_is_rejected(self):
        self.overrides.symlink_to(self.sources)
        self._assert_rejected("overrides: symlinked directory refused")

    def test_rendered_override_passes_through_existing_guards(self):
        cases = [
            (
            ...
            (
                "---\nname: demo\ndescription: Public demo.\n---\nUse `hidden`.",
                "references unpublished skill 'hidden'",
            ),
        ]
        directory = self._write_skill(self.overrides, "demo", "Public demo.", "Body.")
        for content, message in cases:
            with self.subTest(message=message):
                (directory / "SKILL.md").write_text(content, encoding="utf-8")
                errors, _ = self._build()
                self.assertTrue(any(message in error for error in errors), errors)

    def test_undiscovered_override_is_an_error_without_falling_back(self):
        self._write_skill(self.overrides, "demo", "Public demo.", "Body.")
        errors, _ = sync.build(
            self.out,
            manifest_path=self.manifest,
            skills_root=self.sources,
            loader=lambda: (
                lambda root: [] if root == self.overrides else self._discover(root),
                self._render,
            ),
        )
        self.assertIn(
            "demo: not rendered (missing from discover_skills output)", errors
        )

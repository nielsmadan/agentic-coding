...
- `launchd/` - LaunchAgents that `sync.sh` symlinks into `~/Library/LaunchAgents/` and loads
- `codex/sync_superpowers.py` - pins installed Superpowers skills to explicit invocation
- `codex/strip_nono_block.py` - removes the nono codex pack's block from `~/.codex/config.toml`
...
Unmarked skills — 47 of 49 — pass through untouched, so the mechanism costs nothing
...
this repo's `codex/sync_superpowers.py` work line-wise, using `tomllib` only to validate the result. The
...
**loadout evicts nono's `developer_instructions` itself.** Everything the old
`sync_config.py` merged — `[mcp_servers.*]`, `[plugins.*]`, `[marketplaces.*]`, the model
defaults — is loadout's, and so is this: `loadout/defaults/codex.json` lists the key under
`$remove`, which owns it in order to delete it. The guidance it used to hold moved to
`loadout/instructions/sandbox.md`, where all four harnesses get it rather than Codex alone.
`nono update` puts the block back; the next `loadout sync` strips it, body and all.

Deleting rather than replacing is the choice that matters: one document to maintain instead of
...
| `droid-local` | allow `~/.factory` | droid's whole state tree: sessions, logs, cache, and the `bin/rg` it shells out to |

**droid has no vendor pack.** `nono search droid` and `nono search factory` both return nothing, so `droid-local` extends `default` rather than a `nolabs-ai/*` pack and spells out the groups itself. Its containment was measured rather than inherited: `~/wrksp` and `~/.factory` writable, `~/ac` read-only, `~/Documents`, `~/.ssh` and `login.keychain-db` denied — the same boundary the other four get.

**A sandboxed droid cannot use the interactive login.** Droid stores that session through keytar, i.e. the macOS login keychain, which stays denied on purpose — `droid doctor` inside the sandbox reports `no usable credentials found` while the same command outside reports `authenticated as …`. Droid therefore authenticates with an API key ([Factory's own instruction](https://docs.factory.ai/droid-cli/cli-reference) is `export FACTORY_API_KEY=fk-...`, generated at app.factory.ai/settings/api-keys), held in the sops store as `FACTORY_API_KEY` and injected by `sops-exec`. Droid reports it as `using FACTORY_API_KEY (overrides any stored login session)`, so the key wins wherever both exist. **There is no `droid-raw`** — droid is sandbox-only, like pi and opencode, which also means it cannot edit `~/ac` or `~/rc`. `FACTORY_DISABLE_KEYRING=1` changes nothing measurable and is not set.
...
- **No agent has keychain access, and TLS does not need it.** This took two mistakes to get right. Our profiles carried a narrow `read_file` grant on `login.keychain-db`, which looked like the whole story; underneath it the vendor packs' groups (`claude_code_macos`, `codex_macos`) and the `nolabs-ai/opencode` profile granted the keychain **read+write**, and `security show-keychain-info` worked from inside the sandbox against an unlocked keychain. The narrow entry was in fact *narrowing* the group grant, so removing it alone made `login.keychain-db` writable. The grants are dropped at their source instead: `claude-local` no longer includes `claude_code_macos` (its only non-keychain grant, the URL handler app, is re-added explicitly), `codex-local` reproduces the pack profile without `codex_macos` or its keychain entries, and `opencode-local` reproduces the pack profile without its `read_file` entry. Reproducing is what holds across pack updates: `nolabs-ai/codex` 0.2.3 moved the keychain grant out of `codex_macos` and into its own profile, with `bypass_protection`, so a `codex-local` that still extended the pack and only excluded the group could read the login keychain again (18 identities instead of 3) the moment the pack updated. `groups.exclude` works where `filesystem.deny` cannot — a deny does not override an inherited grant ([nono#727](https://github.com/nolabs-ai/nono/issues/727)), and that was measured here too. Codex's Rust TLS stack read trust settings from that keychain, which is why removing it once produced `invalid peer certificate: UnknownIssuer`; `SSL_CERT_FILE=/etc/ssl/cert.pem` in the sandbox wrapper supplies the same public roots and all four agents reach the network with the keychain fully denied. A CA installed **only** in the login keychain — a corporate MITM root, a local dev cert — will not be trusted by sandboxed agents.
...
- **`deny_credentials` paths need `bypass_protection`, not just a grant.** `~/.npmrc` and `~/.netrc` sit in nono's permanent deny group: a `read_file` entry alone still yields `filesystem_deny`, and only adding the path to `bypass_protection` opens it. Weigh that against exfiltration before opening one: sandboxed agents have open outbound network, so a readable credential file is an exfiltratable one. **`~/.npmrc` is not granted; it is kept absent instead, which is strictly safer.** yarn 1.22 opens it on *every* invocation — `yarn node --version` alone fails with `EPERM: operation not permitted, open '/Users/nielsmadan/.npmrc'` — so any repo whose pre-commit hook runs yarn could not be committed from a sandbox at all, while repos whose hooks do not (this one) were unaffected, which is why it looked session-specific. Unlike the `uvx` case there is no redirect: yarn resolves the path through `os.homedir()`, so neither `npm_config_userconfig` nor overriding `HOME` moves it (both measured still opening the literal path). Granting it would work, but the decisive measurement is that **an absent path returns `ENOENT`, not `EPERM`** — and yarn tolerates `ENOENT` and proceeds. So the token moved to `~/.npmrc-publish` (`npm publish --userconfig ~/.npmrc-publish`) and `~/.npmrc` was deleted. That needs no standing grant, and it fails *closed*: if `npm login` recreates the file, the deny group applies again and yarn breaks loudly rather than a credential silently becoming readable by every agent. **An empty file is not enough** — the deny is on the path, so a zero-byte `~/.npmrc` still yields `EPERM`. `sync.sh`'s `warn_if_npmrc_exists` reports the file whenever it reappears, with different advice depending on whether it holds a credential.
- **A `read` grant is enough to execute a binary — never use `allow` to make one runnable.** Measured with hooklinesinker: `read` on `$HOME/.local/share/hooklinesinker` execs the hook binary (`rc=0`) while writes into its `bin/` stay denied. Agent hooks run binaries the user's *own* unsandboxed sessions also run, so a writable install directory would let one agent replace a binary every later session executes. Grant the install tree `read` and only its state directory `allow` — and grant the parent of `bin/`, since the entry point is typically a symlink into a sibling `versions/` directory.
- **Flutter is the deliberate exception to that rule.** `~/development/flutter/bin/cache` must be writable: `bin/internal/update_engine_version.sh` rewrites `engine.stamp.tmp.<random>` and `engine.realm` on every invocation, and a random suffix matches no single-file grant (the same shape that moved `~/.claude.json` into `~/.claude`). That directory holds `dart-sdk/bin/dart` and `flutter_tools.snapshot`, which the user's own unsandboxed `flutter` executes, so the exposure is accepted rather than overlooked — the alternative is a second 4 GB SDK under `~/wrksp` for agents only. `flutter analyze` additionally needs `$HOME/.dartServer`; without it the tool exits 255 with a `getStateLocation` stack trace and **no path in the denial footer**.
- **Claude Code's background daemon is disabled on purpose** — `disableAgentView` in `loadout/settings/claude.json`. A background session inherits the Seatbelt policy of whichever process cold-started the supervisor and then serves every later client, so a daemon first started from `~/ac` or `~/rc` (where `AGENT_RAW_DIRS` routes to `-raw`) ran later `/fork` and `--bg` workers **unsandboxed** even when the requesting session was sandboxed. It could not cold-start sandboxed either: the supervisor spawns with `cwd=$HOME`, which is ungranted, and there `getcwd()` and `readdir()` fail while `stat()` succeeds — so the bug was forcing the least-contained configuration. nono cannot grant `$HOME` to fix it (state-root overlap, from a profile as well as the CLI), and `read_file` refuses a directory outright. The setting is checked inside `daemon run` itself (`background agents disabled (3P/opt-out)`), so the path is unreachable rather than merely unused. **`/fork` is not lost** — the feature gate swaps it to an in-process variant ("Spawn a background agent that inherits the full conversation") that inherits the parent's sandbox by construction; only the detached form goes, along with `claude agents` and `--bg`. Cross-session messaging is unaffected: it uses per-PID sockets in `/tmp/cc-socks/`, not the daemon.
- **A blocked *system service* is not a path denial, and no grant can fix one.** nono's exit footer has a second section, `Also blocked (system services)`, listing operations rather than paths. `xcrun devicectl device install app` failed there for hours with `The item at FlowLab.app is not a valid bundle` — the bundle was valid, readable, and `nono why` said allowed; what was blocked was `file-issue-extension`, Seatbelt's operation for issuing a sandbox extension, which devicectl needs so Apple's install daemon can reach the bundle. `agent-common.json` now allows it for the two app-sandbox classes. Two rounds of path grants were spent before the footer was read, and the footer was unreachable from inside the failing session because `_agent_sandboxed` adds `--silent` for `claude-local`. **To diagnose this class, re-run the command from an unsandboxed session under `nono run -p claude-local` without `--silent` and read the footer** — nothing inside a sandboxed session can see it, and `nono audit`/`nono logs` do not record denials either.
- **pnpm's `packageManager` pin needs two writable directories, for the same reason Flutter does.** A repo pinning `"packageManager": "pnpm@9.15.4"` makes pnpm self-install that exact version before running *any* command — `pnpm --version` alone fails without the grant. Granted: `$HOME/Library/pnpm/global` and `$HOME/Library/pnpm/package-manager-store`; the `~/Library/pnpm` root and its `bin/` stay denied, matching how `.pub-cache` and `.yarn` are handled. The store holds `links/@pnpm/exe/macos-arm64`, a native binary the user's *own* unsandboxed pnpm executes in those repos, so the exposure is accepted rather than overlooked. Setting `manage-package-manager-versions=false` removes the need for any grant but runs the machine's pnpm (12.x) against repos pinning 9.x, risking a `pnpm-lock.yaml` rewrite in a format the pinned version cannot read — worse than the vector it avoids. **`sync.sh` seeds both directories**: nono skips a grant whose path does not exist, and pnpm cannot then create it.
...
- **Check a new nono release against every profile before switching to it.** 0.77 turned legacy profile keys from warnings into parse errors (`undo` → `rollback`; `interactive` removed outright, as it had no effect), and the warnings never reached us because Claude runs nono with `--silent`. Installing 0.78.0 made all five profiles fail with `unknown field 'undo'`, so no sandboxed agent could start. Run `$(mise where github:nolabs-ai/nono@<version>)/nono run --silent -p <profile> -- /usr/bin/true` for each profile against the new binary first; the vendor packs need `nono update` in the same step, since they carry the old keys too.
- **The claude and codex packs write into generated files.** The claude pack `json_merge`s `enabledPlugins` into `~/.claude/settings.json`, which is why `nono@nolabs-ai` is in `loadout/settings/claude.json`. The codex pack appends a `toml_block` to `~/.codex/config.toml`; despite its `position: "top"` it lands at the end of the file, where its top-level `developer_instructions` key gets absorbed into whatever table is last, so it is not even top-level when nono writes it. When that table is one Codex validates strictly (it was `[tui.keymap.chat]` after the 0.2.3 update), Codex refuses the whole file with `unknown field 'developer_instructions'`. Worse, on its next update nono deletes everything between its markers, and Codex had written `notify`, `service_tier` and every `[plugins.*]` / `[marketplaces.*]` entry in there. loadout **removes** it rather than replacing it, via `$remove` in `loadout/defaults/codex.json`: the pack's version tells Codex to treat any `Operation not permitted` as a nono boundary and to offer `nono run --allow` / `nono profile promote`, which produced four false denial reports in a day, and the corrected guidance now lives in `loadout/instructions/sandbox.md` where every harness gets it. `$remove` only reaches a top-level key, so `./sync.sh` also runs `codex/strip_nono_block.py`, which deletes the markers and the key wherever the block landed and leaves every other line alone. A `nono update` restores the pack's copy; the next `./sync.sh` strips it again, so this self-heals. **After a `nono update`, run `./sync.sh`, then `bin/nono-audit --check`, and confirm `login.keychain-db` is still unreadable under every profile** — a pack update changes the grants a profile inherits, and the audit covers only writable paths.

...
3. Verify that exact path with `nono why --self --path <path> --op read|write|readwrite`. `--self` is required: without it nono why evaluates the DEFAULT profile and reports DENIED for almost anything, even outside a sandbox. It also ignores `bypass_protection`, so it reports DENIED for paths that in fact read fine. If it says denied but the command works, believe the command.

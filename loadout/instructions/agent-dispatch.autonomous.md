...
Choose the smallest useful set of agents for independent work. Use the harness's concurrency limit and queue further work as slots become available. An invoked skill authorizes its documented workflow. Allocate autonomous work up front and expand only to resolve a specific remaining requirement within the task's runtime and spending limits.

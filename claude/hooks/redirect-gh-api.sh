...
# The comment/review rules below redirect to read-only subcommands, and gh has no native
# command for line-level review comments — blocking a write leaves no way to post one.
is_write=""
echo "$command" | grep -qE '(--method|-X)[[:space:]]+(POST|PATCH|PUT|DELETE)' && is_write=1

# Issue comments: gh api repos/{owner}/{repo}/issues/{num}/comments
if [ -z "$is_write" ] && echo "$command" | grep -qE 'repos/[^/]+/[^/]+/issues/[0-9]+/comments'; then
...
if [ -z "$is_write" ] && echo "$command" | grep -qE 'repos/[^/]+/[^/]+/pulls/[0-9]+/comments'; then
  echo "BLOCKED: Use 'gh pr view <num> --comments -R <owner/repo>' instead of gh api for PR comments (to POST a line-level comment, re-run with --method POST)." >&2
  exit 2
fi

# PR reviews: gh api repos/{owner}/{repo}/pulls/{num}/reviews
if [ -z "$is_write" ] && echo "$command" | grep -qE 'repos/[^/]+/[^/]+/pulls/[0-9]+/reviews'; then
  echo "BLOCKED: Use 'gh pr view <num> --json reviews -R <owner/repo>' instead of gh api for PR reviews (to POST a review, re-run with --method POST)." >&2
  exit 2
fi

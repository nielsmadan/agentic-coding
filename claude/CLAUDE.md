...
The session is **stateful and persists across invocations** until `close`, so each command above is its own shell call against the same live browser. Reach for this over `WebFetch`/Jina whenever the page needs JS execution, interaction, or auth you already have in a logged-in session — `WebFetch`/Jina stay the right call for read-only fetching of static external pages.
